...
const getPointsConfig = (currentRound: number, totalRounds: number) => {
    let multiplier = 1;
...
const CountUp = ({ value, from }: { value: number; from: number }) => {
...
const PointsPopup = ({ amount, label = 'PTS' }: { amount: number, label?: string }) => {
    useEffect(() => {
        sfx.play('SUCCESS');
    }, []);
...
// --- EMOTE AVATAR POPUP ---
const EmotePopupLayer = ({ emotes }: { emotes: Emote[] }) => {
    return (
...
// --- CATEGORY ROULETTE ---
const CategoryRoulette = ({ state }: { state: GameState }) => {
...
const RevealSequence = ({ state, actions, setGalleryOverrides }: { state: GameState, actions: any, setGalleryOverrides: (o: Record<string, Expression>) => void }) => {
...
const LeaderboardSequence = ({ state, actions, onHome }: { state: GameState, actions: any, onHome: () => void }) => {
...
export const HostView: React.FC<HostViewProps> = ({ state, actions, onHome, debugMode }) => {
  ...
  const renderLobby = () => (
    <div className="flex flex-col items-center justify-center h-full space-y-8 z-20 relative">
      ...
      {!debugMode && (
          <div className="text-xl text-yellow-400 font-bold animate-pulse uppercase mt-8">
  ...
  return (
    <div className="relative w-full h-full bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 overflow-hidden text-white selection:bg-pink-500 font-display flex flex-col">
      ...
      <main className="flex-1 w-full p-8 z-20 relative flex flex-col">
          ...
          {state.phase === GamePhase.REVEAL && <RevealSequence state={state} actions={actions} setGalleryOverrides={setGalleryOverrides} />}
          {(state.phase === GamePhase.LEADERBOARD || state.phase === GamePhase.GAME_OVER) && <LeaderboardSequence state={state} actions={actions} onHome={onHome} />}
      </main>
